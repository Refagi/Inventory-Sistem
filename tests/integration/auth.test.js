...
const { userOneAccessToken, insertRefreshToken, refreshOne, userOneRefreshToken, userTwoRefreshToken} = require('../fixtures/token.fixture');
...
describe('Auth routes', () => {
  ...
  describe('PATCH /api/auth/logout', () => {
    test('should return 200 and logout user if email match', async () => {
      ...
      const res = await request(app).patch('/api/auth/logout').send(logoutCredentials);
  ...
  describe('POST /api/auth/refresh', () => {
    ...
    test('should return 200 and new token if refresh token is valid', async () => {
      ...
      const refreshCredentials = { refreshToken: userOneRefreshToken};
      ...
      expect(dbToken.token).not.toBe(refreshOne.token); //memastikan token sudah di ganti
     });
